...
```
  ...
  - BRFSS data shows the health status of different locations across
    ...
    good, good, fair, or poor. There are rows in this dataset. The
    median response value for the dataset is NA.
...
ggplot(excellent_response, aes(x = year, y = data_location)) + 
  ...
  labs(
    title = "Average Response Over Time in All States",
    x = "Year",
